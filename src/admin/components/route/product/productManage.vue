  ...
  <div id="product-manage">
        ...
        <div class="productManage-table right-content-index">
            ...
            >
                ...
                <el-table-column label="操作处理" align="center" width="150">
                    <template slot-scope="scope">
                        <el-tooltip content="上架" placement="bottom" effect="light">
                           <span class="does up" @click="upStore(scope.$index, scope.row)" v-show="state==='0'"></span>
                        </el-tooltip>
                        <el-tooltip content="下架" placement="bottom" effect="light">
                           <span class="does down" @click="downStore(scope.$index, scope.row)" v-show="state==='1'"></span>
                        </el-tooltip>
                        <el-tooltip content="删除" placement="bottom" effect="light">
                           <span class="does delete" @click="handleDelete(scope.$index, scope.row)"></span>
                        </el-tooltip>
                        <el-tooltip content="详情" placement="bottom" effect="light">
                           <span class="does detail" @click="getDetail(scope.$index, scope.row)"></span>
                        </el-tooltip>
...
export default {
    ...
    methods: {
        ...
        loadData(){
            this.$http.axios({
                url: this.$api.getRouteCommandList+'?start='+(this.currentPage-1)+'&size='+this.pagesize+'&proStored='+this.state+'&createTime='+this.year+'-01-01',
                method: 'post',
                data: '',
                json: true
            }).then(res=>{
