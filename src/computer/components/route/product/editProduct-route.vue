	...
	<div class="route-arrange">
		...
		<div class="content">
			...
			<div class="button-area">
	            <el-button type="primary" plain size='middle' style='' @click="nextFour" :disabled='isTrue'>提交</el-button>
	        </div>
        </div>
	</div>
</template>
	...
	export default{
		...
		methods:{
			...
			nextFour(){//下一步
				...
				this.isTrue=true;
				this.$http.axios({
	                url:this.$api.updateRouteProThird+'?proNum='+this.SproNum,
	                method:'post',
	                data:obj,
                    json:true,
	            }).then(res=>{
		...
		computed: {
			...mapState('editRoutePro',['basic','SproNum','route'])
		    // editor() {
