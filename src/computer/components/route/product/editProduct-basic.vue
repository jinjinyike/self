<template>
	<div style="padding-top:10px">
		...
		<div class="item">
			...
			<el-select v-model="proType3" placeholder="请选择" style='width:25%' :disabled='true'>
		...
		<div class="item">
			...
			<el-select v-model='proDays' style="width:18.2%" :disabled='true'>
		...
		<div class="item">
			...
			<el-checkbox-group v-model="priceArr" class="item-left" :disabled='true'>
		...
		<div class="item">
			...
			<el-select v-model="isContainHotel" placeholder="请选择" :disabled='true'>
		...
		<div class="item" v-if='isContainHotel==1'>
			...
			<el-select v-model="isRoomFree" placeholder="请选择" :disabled='true'>
		...
		<div class="item">
			...
			<el-select v-model="proAuditEmployee " placeholder="请选择">
			...
			<el-select v-model="proOrderInfo " placeholder="请选择" style='width:20%'>
				<el-option v-for="item in employeeArr" :key="item.value" :label="item.label" :value="item.value" :disabled="item.disabled">
				</el-option>
			</el-select>
		</div>
		...
		<div class="item">
			...
			<template v-if='proStoredSetting==1'>
				...
				<!-- <span class="span-title right-title">上线时间</span> -->
		...
		<div class="button-area">
			<!-- <el-button type="primary" plain size='middle' @click='oneSave' :disabled='save'>提交</el-button> -->
			<el-button type="primary" plain size='middle'  @click='nextSecond' :disabled='commit'>提交</el-button>
		</div>
	</div>
	<!-- </div> -->
...
export default {
	data() {
		return {
			...
			// options9: [{ value: '0', label: '不限员工' }, { value: '001', label: '张三' }],

			// options10: [{ value: '0', label: '不设提醒' }, { value: '001', label: '李三' }],
			options11: [
	...
	computed: {
		...mapState('editRoutePro', ['SproNum', 'basic','employeeArr']),
		age() {
			var ageArr = ['不限年龄'];
			for (let i = 1; i <= 100; i++) {
				ageArr.push(i);
		...
		basic: {
			handler() {
				...
				if (this.basic.routeProEntity) {
					...
					// this.proStartCity = '化合[150100]';
					// console.log(this.proStartCity.split('[')[1])
					if(obj.proStartCity){
					...
					if(this.proType22==false&&this.customCitys.length>1){
							if (this.contactServe == 1 && this.contactArr.length > 0) {
								this.contactArr.forEach(function(item2, index2) {
									// console.log(vm.$refs.cityPicker3[index2])
	...
	methods: {
		...
		pickData() {
			...
			obj.proStartCity = this.proStartCity;
		...
		flagData() {
			...
			if (!this.proDestinationArr.length) {
		...
		// ...mapActions('addPro', ['oneCommit','twoCommit'])
	},
	components: {
		cityPicker
	}
