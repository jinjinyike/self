	...
	<div id="tour-manage">
        <div class="manage-top">
            <div class="line">
                ...
                <template>
                    <el-radio v-model="splitType" label="0" @change="radioChange">未报账旅游团</el-radio>
                    <el-radio v-model="splitType" label="1" @change="radioChange">已报账旅游团</el-radio>
                </template>
            </div>
        </div>
		<div class="productManage-table right-content-index">
            <div class="mid-line">
                ...
                <div class="audit">
                    ...
                    <el-button class="" type="primary" plain size='small' @click="goToInfo" v-show="splitType==='0'">开始报账</el-button>
                    <el-button class="" type="primary" plain size='small' @click="goToInfo" v-show="splitType==='1'">重新报账</el-button>
                </div>
            </div>
			<el-table ref="testTable" :data="tableData" style="width:100%" @selection-change="handleSelectionChange" @row-click="handleclick" :row-class-name="tableRowClassName">
				...
				<el-table-column label="报账状态" width="100" align="center" >
					<template slot-scope="scope">
						<span v-show='tableData[scope.$index].isBill==="0"'>未报账</span>
						<span v-show='tableData[scope.$index].isBill==="1"'>已报账</span>
					</template>
				</el-table-column>
...
export default {
	...
	methods: {
        ...
        radioChange(){
            // 已通知未通知切换
			this.loadData();
			if (this.splitType==='0') this.tourText = '未报账';
            if (this.splitType==='1') this.tourText = '已报账';
        },
        goToInfo(){ // 点击开始通知
			...
			if(this.multipleSelection.length!=1){
				...
				this.$router.push({
					...
					query:{teamId:this.multipleSelection[0].teamId}
				})
			}
        },
