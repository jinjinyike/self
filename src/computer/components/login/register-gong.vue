	...
	<div class="gong">
		<div class="page1" v-if="pageInfo">
			<el-steps :active="stepNum" align-center>
				...
				<el-step title="2.申请成为供应商">
					<i slot="icon" class="icon2"></i>
				</el-step>
				<el-step title="3.签订合作协议">
				...
				<table>
					<tr>
						...
						<td>
							...
							<p>
								...
								<span style="color:red;width:200px;text-align:left;">*{{account.phoMsgInfo}}</span>
							...
							<p>
								...
								<el-input placeholder="输入验证码" v-model.trim="account.code" style='width:130px;' :disabled='editorCode' :maxlength='6' @keyup.native="account.code=account.code.replace(/\D/g,'')">
									<!-- <el-input placeholder="输入验证码" v-model.trim="account.code" style='width:130px;' :disabled='editorCode' :maxlength='6' @blur='confirmTelCode' @keyup.native="account.code=account.code.replace(/\D/g,'')"> -->
								</el-input>
					...
					<tr>
						...
						<td>
							...
							<p>
								<span>联系电话</span>
								<el-input placeholder="填写联系人手机号码" v-model.trim="account.contactPho" :maxlength='11' @keyup.native="account.contactPho=account.contactPho.replace(/\D/g,'')">
								</el-input>
...
export default {
	...
	methods: {
		...
		submitInfo() {
			...
			if (this.companyType.select.length === 0) {
				this.$message.error('请填写公司类型');
				return;
			}
			if (this.account.contactName.length === 0) {
				this.$message.error('请填写联系人姓名');
				return;
			}
			if (this.account.contactPho.length !== 11) {
				...
				this.$http
					.axios({
						...
						data: {
							...
							contactPhone: this.account.contactPho,
							userRole: this.companyType.select
						}
					})
					.then(res => {
						if (res === 1) {
