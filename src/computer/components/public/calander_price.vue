		    ...
		    <ul class="days">
		        ...
		        <li  v-for="dayobject in days" @click="data(dayobject,days)" :class='{blue:dayobject.choosed}'>
		            ...
		            <span v-if="dayobject.day.getMonth()+1 == currentMonth" >
                        ...
                        <span  class='red' v-else-if='dayobject.price'>{{ dayobject.day.getDate() }}</span>
        ...
        methods: {
            initData: function(cur) {
                ...
                //初始化本周
                   let obj = [];
                //    obj=[...this.obj];//复制数组
                // obj=Object.assign({},this.obj);//浅拷贝对象
                // obj=this.clone(this.obj);//克隆对象
                for(let i=0;i<this.obj.length;i++){
                	obj[i] = Object.assign({},this.obj[i])
                }
                obj.forEach(item=>item.day=item.day.split('-'));//简写
            ...
            data(dayobject,days){
                if(dayobject.day.getMonth()+1== this.currentMonth&&dayobject.price && this.canClick){
